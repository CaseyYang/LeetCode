...
//	string s;
//	TNode *pre;
//	TNode(string s, TNode *pre) :s(s), pre(pre){}
//};
//class Solution {
//public:
//	vector<vector<string>> findLadders(string start, string end, unordered_set<string> &dict) {
//		vector<vector<string>> results = vector<vector<string>>();
//		vector<TNode*> lastNodes = vector<TNode*>();
//		queue<TNode*> q = queue<TNode*>();
//		TNode *head = new TNode(start, NULL);
//		q.push(head);
//		q.push(new TNode("#", NULL));
//		if (dict.find(start) != dict.end()){
//			dict.erase(start);
//		}
//		vector<string> toBeErased = vector<string>();
//		bool findAResult = false;
//		while (!q.empty()){
//			TNode *curNode = q.front();
//			q.pop();
//			if (curNode->s == "#"&&!q.empty()){
//				if (findAResult){
//					break;
//				}
//				else{
//					for (int i = 0; i < toBeErased.size(); ++i){
//						dict.erase(toBeErased[i]);
//					}
//					toBeErased.clear();
//					q.push(curNode);
//				}
//			}
//			else{
//				for (int i = 0; i < curNode->s.size(); ++i){
//					char curChar = curNode->s[i];
//					for (int j = 0; j < 26; ++j){
//						if ('a' + j != curChar){
//							curNode->s[i] = 'a' + j;
//							if (curNode->s == end){
//								findAResult = true;
//								lastNodes.push_back(new TNode(end, curNode));
//							}
//							else{
//								if (dict.find(curNode->s) != dict.end()){
//									TNode *canad = new TNode(curNode->s, curNode);
//									toBeErased.push_back(curNode->s);
//									q.push(canad);
//								}
//							}
//						}
...
//			TNode *curNode = lastNodes[i];
//			while (curNode != NULL){
//				result.push_back(curNode->s);
//				curNode = curNode->pre;
//			}
...
public:
	...
	void DFS(unordered_map<string, vector<string>> &pres, vector<string> &result, const string &lastWord){
		if (pres[lastWord].size() == 0){
			...
			const vector<string> &curPres = pres[lastWord];
			result.push_back(lastWord);
			for (vector<string>::const_iterator citr = curPres.begin(); citr != curPres.end(); citr++){
				DFS(pres, result, *citr);
			}
			result.pop_back();
		}
	}
	vector<vector<string>> findLadders(string start, string end, unordered_set<string> &dict) {
		unordered_map<string, vector<string>> pres = unordered_map<string, vector<string>>();
		...
		while (true){
			cur = !cur;
			pre = !pre;
			for (unordered_set<string>::iterator iter = layers[cur].begin(); iter != layers[cur].end(); ++iter){
				dict.erase(*iter);
			}
			layers[pre].clear();
			for (unordered_set<string>::iterator iter = layers[cur].begin(); iter != layers[cur].end(); ++iter){
				...
				for (int i = 0; i < curStr.size(); ++i){
					...
					char curChar = tmpStr[i];
					for (int j = 0; j < 26; ++j){
						if ('a' + j != curChar){
							tmpStr[i] = 'a' + j;
							if (tmpStr == end){ 
								pres[tmpStr].push_back(curStr); 
								layers[pre].insert(tmpStr); 
							}
							else{
			...
			if (layers[pre].size() == 0){ return results; }
			if (layers[pre].find(end) != layers[pre].end()){ break; }
		}
		vector<string> result = vector<string>();
		DFS(pres, result, end);
		return results;
...
int main(){
	...
	auto st = clock();
	Solution s;
	vector<vector<string>> results = s.findLadders("a", "c", dict);
	cout << "results" << endl;
